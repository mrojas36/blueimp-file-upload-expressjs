module.exports = function(opts) {
  ...
  function getContentTypeByFile(fileName) {
    ...
    else if (fn.indexOf('.pdf') >= 0) rc = 'application/pdf';
  ...
  fileUploader.delete = function(req, res, callback) {
    ...
    if (options.storage.type == 'local') {
      ...
      s3.deleteObject(params, function(err, data) {
        if (err) console.log("[FileUploader] S3 Error: " + err, err.stack); // an error occurred
        else console.log("[FileUploader] S3 OK: " + JSON.stringify(data)); // successful response
        callback({
          success: data
        });
      });
